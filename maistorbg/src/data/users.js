export default [
    {
      name: 'Мария Маринова',
      username: 'ChloeD12',
      password: 'ChloeD12@#',
      email: 'ChloeD12@gmail.com',
      id: 956015,
      number: 1234567890,
      isClient: true
    },
    {
      name: 'Ивана Иванова',
      username: 'JPatel3',
      password: '9$BdP#o5',
      email: 'JPatel3@gmail.com',
      number: 1345678902,
      isClient: true,
      id: 973399
    },
    {
      name: 'Николай Иванов',
      username: 'MRodriguez7',
      password: '@Tf7G#Ei',
      email: 'MRodriguez7@hotmail.com',
      number: 8756490321,
      isClient: true,
      id: 33613
    },
    {
      name: 'Петър Георгиев',
      username: 'ALee4',
      password: 'Nw7#Bq3J',
      email: 'ALee4@yahoo.com',
      number: 2998345678,
      isClient: true,
      id: 677094
    },
    {
      name: 'Мартин Николов',
      username: 'ZHuang8',
      password: 'r@D5Xp9W',
      email: 'ZHuang8@gmail.com',
      number: 6410987532,
      isClient: true,
      id: 584852
    },
    {
      name: 'София Георгиева',
      username: 'AKhan6',
      password: 's#Z9fT2Q',
      email: 'AKhan6@hotmail.com',
      number: 1783562490,
      isClient: true,
      id: 620760
    },
    {
      name: 'Александър Георгиев',
      username: 'EChen5',
      password: 'p7$RfM8A',
      email: 'EChen5@yahoo.com',
      number: 8973561240,
      isClient: true,
      id: 23615
    },
    {
      name: 'Марта Мартинова',
      username: 'ADavis2',
      password: 'B@9zN4mT',
      email: 'ADavis2@gmail.com',
      number: 3247890561,
      isClient: true,
      id: 55257
    },
    {
      name: 'Владимир Петров',
      username: 'AKim9',
      password: 'g#F6sQ8W',
      email: 'AKim9@hotmail.com',
      number: 4582931765,
      isClient: true,
      id: 919835
    },
    {
      name: 'Лилия Николова',
      username: 'NNguyen7',
      password: 'V3@qB5eP',
      email: 'NNguyen7@yahoo.com',
      number: 8365214097,
      isClient: true,
      id: 98981
    },
    {
      name: 'Симеон Симеонов',
      username: 'LPatel4',
      password: 's@M6X9hT',
      email: 'LPatel4@gmail.com',
      number: 2398461057,
      isClient: true,
      id: 348559
    },
    {
      name: 'Виктор Димитров',
      username: 'NLee6',
      password: 'T#n6Gq7W',
      email: 'NLee6@hotmail.com',
      number: 7189456230,
      isClient: true,
      id: 995993
    },
    {
      name: 'Кристина Кръстева',
      username: 'IKim5',
      password: 'B8@sF4uP',
      email: 'IKim5@gmail.com',
      number: 8461235790,
      isClient: true,
      id: 974145
    },
    {
      name: 'Димитър Иванов',
      username: 'RChen7',
      password: 'X9@mB7tK',
      email: 'RChen7@yahoo.com',
      number: 6372584190,
      isClient: true,
      id: 11401
    },
    {
      name: 'Надежда Николова',
      username: 'SNguyen9',
      password: 'M#b5V7zE',
      email: 'SNguyen9@hotmail.com',
      number: 9657832410,
      isClient: true,
      id: 717519
    },
    {
      name: 'Иван Димитров',
      username: 'EDavis4',
      password: 'q@3Yp7Jw',
      email: 'EDavis4@gmail.com',
      number: 3876542910,
      isClient: true,
      id: 315656
    },
    {
      name: 'Алексей Алексиев',
      username: 'ABrown6',
      password: 'D#m8Wq5G',
      email: 'ABrown6@hotmail.com',
      number: 5748392106,
      isClient: true,
      id: 172894
    },
    {
      name: 'Любомир Николов',
      username: 'BGarcia2',
      password: 'C@5Pw6rL',
      email: 'BGarcia2@yahoo.com',
      number: 2785931460,
      isClient: true,
      id: 735564
    },
    {
      name: 'Елена Димитрова',
      username: 'CAdams3',
      password: 'J7#bE6xT',
      email: 'CAdams3@gmail.com',
      number: 9056732145,
      isClient: true,
      id: 889028
    },
    {
      name: 'Иван Киров',
      username: 'JHernandez7',
      password: 'X9@tH4bM',
      email: 'JHernandez7@hotmail.com',
      number: 3562890471,
      isClient: true,
      id: 548498
    },
    {
      name: 'Дарина Иванова',
      username: 'APerez9',
      password: 'S8@jK3qZ',
      email: 'APerez9@yahoo.com',
      number: 9784632150,
      isClient: true,
      id: 277604
    },
    {
      name: 'Ангелина Ангелова',
      username: 'SMitchell4',
      password: 'W#p4Gj9X',
      email: 'SMitchell4@gmail.com',
      number: 4827365190,
      isClient: true,
      id: 193821
    },
    {
      name: 'Владислав Владимиров',
      username: 'DCampbell8',
      password: 'F#h9Dq5N',
      email: 'DCampbell8@hotmail.com',
      number: 9256173840,
      isClient: true,
      id: 91940
    },
    {
      name: 'Виктория Василева',
      username: 'OClark1',
      password: 'L@5Cq3tR',
      email: 'OClark1@yahoo.com',
      number: 3810597264,
      isClient: true,
      id: 253639
    },
    {
      name: 'Георги Петров',
      username: 'LBaker6',
      password: 'V#p4Kj8W',
      email: 'LBaker6@hotmail.com',
      number: 6759321480,
      isClient: true,
      id: 895653
    },
    {
      name: 'Десислава Десиславова',
      username: 'MWard3',
      password: 'R#n5Lq6X',
      email: 'MWard3@gmail.com',
      number: 3154786290,
      isClient: true,
      id: 24277
    },
    {
      name: 'Александър Иванов',
      username: 'JohnDoe13',
      password: 'JohnDoe13@',
      email: 'JohnDoe@gmail.com',
      number: 2893456456,
      isClient: false,
      skills: [ 'painter', 'plaster man', 'carpenter' ],
      allRatings: [
        1, 2, 3, 4, 5,
        3, 2, 4, 5, 5
      ],
      averageRating: 3.4,
      id: 558275,
      photo : []
    },
    {
      name: 'Димитър Петров',
      username: 'AdamSmith123',
      password: 'Passw0rd$',
      email: 'AdamSmith123@gmail.com',
      number: 7465839021,
      isClient: false,
      skills: [ 'Plaster Man', 'Carpenter' ],
      allRatings: [
        3, 4, 4, 2,
        1, 5, 2
      ],
      averageRating: 3,
      id: 987315
    },
    {
      name: 'Георги Георгиев',
      username: 'WillDav25',
      password: 'wD$2passw0rd',
      email: 'WillDav25@hotmail.com',
      number: 5369274801,
      isClient: false,
      skills: [ 'Electrician', 'Plumber', 'Welder' ],
      allRatings: [
        5, 5, 4, 3,
        4, 3, 4
      ],
      averageRating: 4,
      id: 443515
    },
    {
      name: 'Иван Иванов',
      username: 'MikeJohnson88',
      password: 'Joh$2passw0rd',
      email: 'MikeJohnson88@gmail.com',
      number: 6498675309,
      isClient: false,
      skills: [ 'HVAC Engineer', 'Carpenter' ],
      allRatings: [
        2, 2, 3, 2,
        1, 1, 2, 3
      ],
      averageRating: 2.12,
      id: 854543
    },
    {
      name: 'Петър Николов',
      username: 'DavidWils0n',
      password: 'dW$2passw0rd',
      email: 'DavidWils0n@hotmail.com',
      number: 6095550147,
      isClient: false,
      skills: [ 'Plumber', 'Welder' ],
      allRatings: [ 5, 4, 5, 4, 5, 5 ],
      averageRating: 4.67,
      id: 878434
    },
    {
      name: 'Стефан Стефанов',
      username: 'ChrisM_77',
      password: 'cM$2passw0rd',
      email: 'ChrisM_77@gmail.com',
      number: 2135550129,
      isClient: false,
      skills: [ 'Electrician', 'Welder', 'Roofer' ],
      allRatings: [
        3, 3, 2, 3, 2,
        3, 3, 3, 2
      ],
      averageRating: 2.78,
      id: 481594
    },
    {
      name: 'Виктор Василев',
      username: 'Matt_Garc1a',
      password: 'Garc$2passw0rd',
      email: 'Matt_Garc1a@hotmail.com',
      number: 4055550167,
      isClient: false,
      skills: [ 'Builder', 'Carpenter' ],
      allRatings: [
        4, 5, 4, 5,
        4, 5, 4
      ],
      averageRating: 4.43,
      id: 875025
    },
    {
      name: 'Николай Николов',
      username: 'EthanMartinez10',
      password: 'Mar10$2passw0rd',
      email: 'EthanMartinez10@gmail.com',
      number: 5035550148,
      isClient: false,
      skills: [ 'Plumber', 'Welder', 'Electrician' ],
      allRatings: [
        3, 3, 3, 3,
        3, 3, 3
      ],
      averageRating: 3,
      id: 309734
    },
    {
      name: 'Кристиян Димитров',
      username: 'AlexRodriguez28',
      password: 'Rod28$2passw0rd',
      email: 'AlexRodriguez28@hotmail.com',
      number: 7575550127,
      isClient: false,
      skills: [ 'Tehnician', 'Electrician' ],
      allRatings: [
        2, 2, 1, 2,
        1, 1, 2, 2
      ],
      averageRating: 1.75,
      id: 461048
    },
    {
      name: 'Мартин Иванов',
      username: 'RyanAnders0n',
      password: 'And$2passw0rd',
      email: 'RyanAnders0n@gmail.com',
      number: 9045550139,
      isClient: false,
      skills: [ 'Builder', 'Carpenter', 'Plaster Man' ],
      allRatings: [
        4, 5, 4, 5,
        4, 5, 4
      ],
      averageRating: 4.43,
      id: 522274
    },
    {
      name: 'Калоян Калчев',
      username: 'MattWils0n',
      password: 'wM$2passw0rd',
      email: 'MattWils0n@hotmail.com',
      number: 9495550153,
      isClient: false,
      skills: [ 'Roofer', 'Welder' ],
      allRatings: [
        3, 3, 2, 3, 2,
        3, 3, 3, 2
      ],
      averageRating: 2.78,
      id: 74751
    },
    {
      name: 'Ивайло Георгиев',
      username: 'Jacob_Lee42',
      password: 'Lee42$2passw0rd',
      email: 'Jacob_Lee42@gmail.com',
      number: 2125550117,
      isClient: false,
      skills: [ 'Electrician', 'Plumber' ],
      allRatings: [ 5, 4, 5, 4, 5, 5 ],
      averageRating: 4.67,
      id: 68715
    },
    {
      name: 'Радослав Радев',
      username: 'WillClark_33',
      password: 'C33$2passw0rd',
      email: 'WillClark_33@hotmail.com',
      number: 6125550142,
      isClient: false,
      skills: [ 'Electrician', 'Welder', 'HVAC Engineer' ],
      allRatings: [
        5, 5, 4, 3,
        4, 3, 4
      ],
      averageRating: 4,
      id: 51954
    },
    {
      name: 'Ангел Ангелов',
      username: 'JacobS87',
      password: 'Smith87#',
      email: 'JacobS87@gmail.com',
      number: 9468273645,
      isClient: false,
      skills: [ 'Welder', 'Roofer', 'Electrician' ],
      allRatings: [
        4, 4, 4, 4, 4,
        4, 4, 5, 5
      ],
      averageRating: 4.33,
      id: 205342
    },
    {
      name: 'Даниел Стоянов',
      username: 'WillJ1',
      password: 'WilliamJ1@',
      email: 'WilliamJ1@yahoo.com',
      number: 7698412305,
      isClient: false,
      skills: [ 'Builder', 'Plaster Man', 'HVAC Engineer' ],
      allRatings: [
        4, 4, 4, 4, 4,
        4, 4, 4, 4, 5
      ],
      averageRating: 4.2,
      id: 417807
    },
    {
      name: 'Васил Василев',
      username: 'EthanB87',
      password: 'Brown87!',
      email: 'EthanB87@hotmail.com',
      number: 9681237450,
      isClient: false,
      skills: [ 'Welder', 'Electrician', 'Plumber' ],
      allRatings: [ 4, 4, 4, 4, 4, 5 ],
      averageRating: 4.17,
      id: 273767
    },
    {
      name: 'Цветан Цветанов',
      username: 'MGarcia10',
      password: 'MG10@',
      email: 'MGarcia10@gmail.com',
      number: 8570643921,
      isClient: false,
      skills: [ 'Tehnician', 'Electrician', 'Carpenter' ],
      allRatings: [ 4, 4, 4, 4, 5 ],
      averageRating: 4.2,
      id: 51126
    },
    {
      name: 'Борислав Борисов',
      username: 'AlexM86',
      password: 'Martinez86*',
      email: 'AlexM86@gmail.com',
      number: 6437192865,
      isClient: false,
      skills: [ 'Painter', 'Electrician', 'HVAC Engineer' ],
      allRatings: [ 4, 4, 4, 5 ],
      averageRating: 4.25,
      id: 508710
    },
    {
      name: 'Любомир Любомиров',
      username: 'DRobinson23',
      password: 'Robinson23#',
      email: 'DRobinson23@hotmail.com',
      number: 9247863510,
      isClient: false,
      skills: [ 'Builder', 'Plaster Man', 'Carpenter' ],
      allRatings: [ 4, 4, 5 ],
      averageRating: 4.33,
      id: 548153
    },
    {
      name: 'Росен Росенов',
      username: 'AlexM86',
      password: 'Martinez86*',
      email: 'AlexM86@gmail.com',
      number: 6437192865,
      isClient: false,
      skills: [ 'Painter', 'Electrician', 'HVAC Engineer' ],
      allRatings: [ 4, 4, 4, 5 ],
      averageRating: 4.25,
      id: 297218
    },
    {
      name: 'Георги Христов',
      username: 'NickW33',
      password: 'Wilson33$',
      email: 'NickW33@gmail.com',
      number: 5482719384,
      isClient: false,
      skills: [ 'Carpenter', 'Electrician', 'Welder' ],
      allRatings: [
        1, 2, 3, 4, 5,
        3, 2, 4, 5, 5,
        3, 4
      ],
      averageRating: 3.5833333333333335,
      id: 938163
    },
    {
      name: 'Иван Тодоров',
      username: 'WBrown1',
      password: 'Brown1@!',
      email: 'WBrown1@gmail.com',
      number: 7815246398,
      isClient: false,
      skills: [ 'Builder', 'Electrician' ],
      allRatings: [
        1, 2, 3, 4, 5,
        3, 2, 4, 5
      ],
      averageRating: 3.5555555555555554,
      id: 390577
    },
    {
      name: 'Димитър Георгиев',
      username: 'MikeD78',
      password: 'Davis78%',
      email: 'MikeD78@gmail.com',
      number: 1482796352,
      isClient: false,
      skills: [ 'Plaster Man', 'Carpenter', 'Electrician' ],
      allRatings: [
        1, 2, 3, 4,
        5, 3, 2, 4
      ],
      averageRating: 3.375,
      id: 803495
    },
    {
      name: 'Христо Христов',
      username: 'DanGarcia5',
      password: 'Garcia5@#',
      email: 'DanGarcia5@gmail.com',
      number: 6841729358,
      isClient: false,
      skills: [ 'Electrician', 'Roofer', 'Plumber' ],
      allRatings: [
        1, 2, 3, 4, 5,
        3, 2, 4, 5, 5
      ],
      averageRating: 3.9,
      id: 919568
    },
    {
      name: 'Павел Павлов',
      username: 'CRodriguez17',
      password: 'Rodriguez17$',
      email: 'CRodriguez17@gmail.com',
      number: 9548317265,
      isClient: false,
      skills: [ 'Welder', 'Electrician', 'Plumber' ],
      allRatings: [
        1, 2, 3, 4, 5, 3,
        2, 4, 5, 5, 3, 4,
        5
      ],
      averageRating: 3.769230769230769,
      id: 130453
    }
  ]
